...
def main():
    ...
    for (dataset, sent_count), split_dir, model, batch_size in dataset_model_choices:
        for sent_len in [16, 32, 48, 64, 80, 96, 112, 128]:
            ...
            args.sent_count = sent_count
            args.split_dir = f'data/power/split-v2/{split_dir}/'
            args.texter_pkl = f'data/power/texter-v2/final/{dataset}_{model}_{sent_len}.pkl'
            args.eval_yml = f'data/power/eval-v1/final/{dataset}_{model}_{sent_len}.yml'

            args.batch_size = batch_size
            args.log_dir = f'runs/final/{dataset}_{model}_{sent_len}/'
            args.model = model
